...
> Design notes for **inbound-sms-devtools-workflow** — a runnable typescript example that Typed Node service for approving staging builds from inbound SMS replies.
...
- **Feature code** — the actual task: sms-release-approval.
...
- `sms.events` — mapped to `GET /v1/sms/events/{id}`.
